...
class ProjectBrainCliTest(unittest.TestCase):
    ...
    def test_baseline_show_returns_latest_project_baseline_artifact(self):
        with tempfile.TemporaryDirectory() as tmp:
            fixture = create_payment_mini_codegraph_project(Path(tmp))
            store_root = str((Path(tmp) / "store").resolve())
    ...
    def test_project_intake_fourth_answer_advances_to_third_party_integrations_question(self):
        with tempfile.TemporaryDirectory() as tmp:
            fixture = create_payment_mini_codegraph_project(Path(tmp))
            store_root = str((Path(tmp) / "store").resolve())
            ...
            self.assertEqual(output["intake"]["status"], "asking")
            self.assertEqual(output["intake"]["next_question"]["slot_key"], "third_party_integrations")
            self.assertIn("第三方", output["intake"]["next_question"]["question"])
            self.assertEqual(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _run_cli(
                [
                    ...
                    str(fixture["project_path"]),
                    "--id",
                    "payment_intake_fifth_answer_cli",
                    "--experience-seed",
                    str(fixture["experience_seed"]),
                ]
            )
    ...
    def test_project_intake_sixth_answer_advances_to_constraints_question(self):
        with tempfile.TemporaryDirectory() as tmp:
            fixture = create_payment_mini_codegraph_project(Path(tmp))
            store_root = str((Path(tmp) / "store").resolve())
            ...
            self.assertEqual(output["intake"]["status"], "asking")
            self.assertEqual(output["intake"]["next_question"]["slot_key"], "constraints")
            self.assertIn("约束", output["intake"]["next_question"]["question"])
            self.assertEqual(
    ...
    def test_project_intake_seventh_answer_advances_to_validation_strategy_question(self):
        with tempfile.TemporaryDirectory() as tmp:
            fixture = create_payment_mini_codegraph_project(Path(tmp))
            store_root = str((Path(tmp) / "store").resolve())
            ...
            _run_cli(
                [
                    ...
                    str(fixture["project_path"]),
                    "--id",
                    "payment_intake_seventh_answer_cli",
                    "--experience-seed",
                    str(fixture["experience_seed"]),
                ]
            )
            ...
            output = _run_cli(
                [
                    ...
                    "answer",
                    "payment_intake_seventh_answer_cli",
                    sixth_answer["intake"]["session_id"],
                    "--answer",
                    "必须兼容历史账单、只能灰度发布、不能改第三方回调协议。",
                ]
            )

            self.assertEqual(output["intake"]["status"], "asking")
            self.assertEqual(output["intake"]["next_question"]["slot_key"], "validation_strategy")
            self.assertIn("验证", output["intake"]["next_question"]["question"])
            self.assertEqual(
                output["intake"]["captured_fields"]["constraints"],
                "必须兼容历史账单、只能灰度发布、不能改第三方回调协议。",
            )
            self.assertEqual(
                output["intake"]["baseline_draft"]["constraints"],
                ["必须兼容历史账单", "只能灰度发布", "不能改第三方回调协议"],
            )

    def test_project_intake_eighth_answer_completes_baseline_validation_strategy(self):
        with tempfile.TemporaryDirectory() as tmp:
            fixture = create_payment_mini_codegraph_project(Path(tmp))
            store_root = str((Path(tmp) / "store").resolve())
            ...
            _run_cli(
                [
                    ...
                    str(fixture["project_path"]),
                    "--id",
                    "payment_intake_eighth_answer_cli",
                    "--experience-seed",
                    str(fixture["experience_seed"]),
                ]
            )
            ...
            output = _run_cli(
                [
                    ...
                    "answer",
                    "payment_intake_eighth_answer_cli",
                    seventh_answer["intake"]["session_id"],
                    "--answer",
                    "补充单元测试、跑支付回调联调、核对一笔真实对账样本。",
                ]
            )
            ...
            self.assertEqual(
                output["intake"]["captured_fields"]["validation_strategy"],
                "补充单元测试、跑支付回调联调、核对一笔真实对账样本。",
            )
            self.assertEqual(
                output["intake"]["baseline_draft"]["validation_strategy"],
                ["补充单元测试", "跑支付回调联调", "核对一笔真实对账样本"],
            )
