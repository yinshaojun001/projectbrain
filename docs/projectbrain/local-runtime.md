...
`intake project` currently starts a minimal four-step onboarding flow: capture the project goal, primary users, core modules, and key flows, while keeping `project-intake-session-latest.json` plus a lightweight `baseline_draft` in sync.
